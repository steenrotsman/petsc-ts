...
struct queue_order
{
    bool operator()(const Pattern &l, const Pattern &r) const { return l.projection.size() < r.projection.size(); }
};
...
private:
    ...
    std::pair<Projection, int> compute_projection_incremental(DiscreteDB &ts, const Pattern &pattern, int item);
